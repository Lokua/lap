...
Lap.ProgressBar.prototype = {

  init: function() {
    var t = this;
    t.draw();
    t.lap.audio.addEventListener('progress', function() {
      t.draw();
    });
    t.lap.audio.addEventListener('timeupdate', function() {
      t.draw();
    });
    return this;
  },
  ...
  draw: function() {
    ...
    if (s.track.draw) {
      ...
      ofs = (s.track.height < s.playhead.height) ? (s.playhead.height-s.track.height)/2 : 0;
      params = [0, ofs, s.width, s.track.height];
    ...
    if (s.progress.draw) {
      ...
      ofs = (s.progress.height < s.playhead.height) ? (s.playhead.height-s.progress.height)/2 : 0;
      params = [0, ofs, tooly.scale(x, 0, 100, 0, s.width), s.progress.height];
      ctx.fillRect.apply(ctx, params);
    ...
    if (s.playhead.draw) {
      ...
      params = [x, 0, s.playhead.width, s.playhead.height];
      ctx.fillRect.apply(ctx, params);
